...
- [x] 详情页：展示食品完整信息（名称、生产日期、保质期、过期日期、状态）
- [x] 食品编辑/删除
...
- **v1.1.0-alpha 发布**：条形码扫描识别功能
  - 接入 apibyte.cn 条形码查询 API，免费无限制
  - 扫码功能集成到添加食品表单（名称输入框右侧扫码图标）
  - 扫描框添加绿色渐变引导线
  - 新增 `/api/barcode/:code` 路由代理 API 查询
  - 新增 dotenv 依赖管理环境变量
  - MVP 功能全部完成 ✅
- **v1.2.0-alpha 发布**：扫码增强 + 自定义弹窗
  - 扫码双模式：条形码（长框+引导线）和二维码（方框无线）
  - 二维码扫描：扫描已有食品二维码自动填充名称
  - 自定义确认弹窗：替换浏览器原生 confirm
  - HTTPS 支持：解决手机浏览器摄像头调用问题
- **v1.2.1-alpha 发布**：手机扫码框优化
  - 根据屏幕宽度动态调整扫码框大小
  - 手机端使用更小的扫码框，改善对焦和识别体验
- **v1.2.2-alpha 发布**：扫码框高度调整
  - 条形码扫码框高度从 80/100 调整为 120/150（手机/电脑）
  - 解决扫码框太窄导致的对焦问题
- **v1.3.0-alpha 发布**：视觉优化 — 底部导航 + 字体 + 风格升级
  - 新增底部胶囊导航栏（手机端），中间凸起圆形"+"按钮
  - 引入霞鹜文楷 + Nunito 字体（Google Fonts）
  - Tailwind v4 @theme 自定义主色调（草绿色系）
  - 卡片增强阴影 + 左侧状态色条（绿/黄/红）
  - 全局背景色改为极淡绿灰（bg-bg）
  - 确认弹窗圆角和阴影与新风格统一
  - 详情页返回按钮改为胶囊形带图标
